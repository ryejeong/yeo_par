...
const NaverMap = () => {
  ...
  return (
    ...
    // 카카오맵 길찾기 모바일 웹 주소를 활용해 카카오내비 앱 구동을 유도합니다.
    // epName(목적지명), epX(경도), epY(위도)
             const naviUrl = `https://map.kakao.com/link/to/엘마리노 앳 인천,${WEDDING_HALL_POSITION[1]},${WEDDING_HALL_POSITION[0]}`
              switch (checkDevice()) {
               case "ios":
               case "android":
                if (kakao) {
          // 소수점 자리수를 6자리로 강제 제한하여 카카오내비의 좌표 인식 오류(20417)를 방지합니다.
                  const xCoord = Number(WEDDING_HALL_POSITION[0].toFixed(6));
                  const yCoord = Number(WEDDING_HALL_POSITION[1].toFixed(6));
                  kakao.Navi.start({
                    name: LOCATION, // const.ts의 식장 이름
                    x: xCoord,
                    y: yCoord,
                    coordType: "wgs84",
                  });
